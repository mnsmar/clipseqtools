...
=item 8. annotate_with_conservation   - Annotate alignments with PhyloP conservation scores.
...
Finally, one more field is added that contains the conservation score for each read. This score is calculated as the average PhyloP conservation score of the read's nucleotides. The database field is named conservation. To minimize storage needs, the PhyloP conservation score is converted from floating point number to integer by multiplying with 1000.
...
To run an analysis only on reads that are highly conserved, have a deletion, are not repeats and are longer than 31 nucleotides the following flags should be given when running the command:

  --filter conservation=">500" --filter deletion="def" --filter rmsk="undef" --filter query_length=">31"
