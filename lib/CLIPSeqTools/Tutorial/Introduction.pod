...
=item 2. clipseqtools-plot (Helper application to create plots for the output of C<clipseqtools>. Usually, the plots are created via C<clipseqtools> with the C<--plot> option.)
...
Let's assume you have two libraries on which you run C<clipseqtools>.
